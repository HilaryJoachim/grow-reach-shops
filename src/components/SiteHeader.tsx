            ...
            {shopOpen && (
              <div className="absolute left-0 top-full pt-2 w-44">
                <div className="bg-background border border-border rounded-md shadow-lg overflow-hidden">
                  <Link
                    to="/shop"
                    className="block px-4 py-2.5 text-sm hover:bg-muted hover:text-primary"
                  >
                    All Products
                  </Link>
                  {SHOP_GROUPS.map((g) => (
                    <Link
                      key={g.label}
                      to="/shop"
                      search={{ category: g.category } as never}
                      className="block px-4 py-2.5 text-sm hover:bg-muted hover:text-primary"
                    >
                      {g.label}
                    </Link>
      ...
      {open && (
        <div className="lg:hidden border-t border-border bg-background">
          <nav className="container mx-auto flex flex-col px-4 py-3">
            <Link to="/" onClick={() => setOpen(false)} className="py-2.5 border-b border-border">
              Home
            </Link>
            <Link
              to="/shop"
            ...
            {SHOP_GROUPS.map((g) => (
              <Link
                ...
                search={{ category: g.category } as never}
                onClick={() => setOpen(false)}
                className="py-2.5 pl-4 border-b border-border text-muted-foreground"
              >
                {g.label}
              </Link>
            ))}
            ...
            <Link to="/contact" onClick={() => setOpen(false)} className="py-2.5">
              Contact
            </Link>
          </nav>
        </div>
