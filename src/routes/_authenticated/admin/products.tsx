...
import { deleteProduct, listProductsAdmin, upsertProduct, listCategoriesAdmin } from "@/lib/admin.functions";
...
function blank() {
  return {
    ...
    retail_price: 0, wholesale_price: 0, moq: 1,
...
function ProductsAdmin() {
  ...
  function startEdit(p: ProductRow) {
    setEditing({
      ...
      category_slug: p.category_slug, description: p.description,
      benefits: p.benefits, usage: p.usage,
      retail_price: Number(p.retail_price), wholesale_price: Number(p.wholesale_price), moq: p.moq,
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="flex justify-between items-center mb-6">
        ...
        <Button onClick={() => setEditing(blank())}><Plus className="h-4 w-4" /> Add product</Button>
      </div>
      ...
      {editing && (
        ...
        >
          <div className="grid sm:grid-cols-2 gap-4">
            ...
            <Field label="Slug (lowercase, dashes)"><Input required pattern="[a-z0-9-]+" value={editing.slug} onChange={(e) => setEditing({ ...editing, slug: e.target.value })} /></Field>
            <Field label="SKU"><Input value={editing.sku ?? ""} onChange={(e) => setEditing({ ...editing, sku: e.target.value })} /></Field>
            <Field label="Category slug">
              <select required className="flex h-9 w-full rounded-md border border-input bg-transparent px-3 text-sm"
                value={editing.category_slug} onChange={(e) => setEditing({ ...editing, category_slug: e.target.value })}>
          ...
          <div className="grid sm:grid-cols-2 gap-4">
            ...
            <Field label="Usage"><Textarea value={editing.usage} onChange={(e) => setEditing({ ...editing, usage: e.target.value })} /></Field>
          </div>
      ...
      <div className="border border-border rounded-lg divide-y divide-border bg-card">
        {(products ?? []).map((p) => (
          <div key={p.id} className="p-4 flex items-center gap-4">
            ...
            <Button size="sm" variant="ghost" onClick={() => { if (confirm("Delete this product?")) remove.mutate(p.id); }}>
              <Trash2 className="h-4 w-4 text-destructive" />
            </Button>
          </div>
        ))}
        {(products ?? []).length === 0 && <div className="p-8 text-center text-sm text-muted-foreground">No products yet.</div>}
      </div>
    </div>
  );
}
