...
export const Route = createFileRoute("/products/$slug")({
  ...
  errorComponent: ({ error }) => (
    <div className="container mx-auto px-4 py-20 text-center">{error.message}</div>
  ),
  notFoundComponent: () => (
      ...
      <div className="grid lg:grid-cols-2 gap-10">
        <div className="aspect-square bg-muted rounded-lg overflow-hidden relative">
          {product.image_url ? (
            ...
            <div className="absolute inset-0 grid place-items-center font-display text-9xl text-primary/30">
              A
            </div>
          )}
        </div>
        ...
        <div>
          ...
          <div className="mt-6 bg-secondary rounded-lg p-5">
            <div className="flex items-baseline gap-3 flex-wrap">
              <div>
                ...
                <div
                  className={`font-display text-2xl ${isWholesale ? "line-through text-muted-foreground" : "text-primary"}`}
                >
                  {formatTsh(product.retail_price)}
                </div>
              </div>
              <div>
                <div className="text-xs text-muted-foreground">Wholesale (MOQ {product.moq}+)</div>
                <div
                  className={`font-display text-2xl ${isWholesale ? "text-primary" : "text-foreground"}`}
                >
                  {formatTsh(product.wholesale_price)}
                </div>
              </div>
            </div>
            <div className="mt-3 text-xs text-muted-foreground">
              {isWholesale
                ? "Wholesale price applied automatically."
                : `Buy ${product.moq - qty} more to unlock wholesale pricing.`}
            </div>
          </div>
          ...
          <div className="mt-4 flex items-center gap-3">
            ...
            <div className="text-sm">
              Total: <span className="font-display text-xl text-primary">{formatTsh(total)}</span>
            </div>
          </div>
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            ...
            <Button
              ...
              size="lg"
              variant="outline"
              className="bg-ink text-ink-foreground hover:bg-primary hover:text-primary-foreground border-ink"
            >
